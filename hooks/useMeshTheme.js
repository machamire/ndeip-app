...
export const MeshThemeProvider = ({ children }) => {
        ...
        if (value) {
          ...
          switch (key) {
            ...
            case THEME_STORAGE_KEYS.ACCESSIBILITY_SETTINGS:
              storedConfig.accessibilitySettings = { ...storedConfig.accessibilitySettings, ...parsedValue };
              break;
  ...
  const saveThemeToStorage = async (config) => {
    try {
      const storageOperations = [
        ...
        [THEME_STORAGE_KEYS.ANIMATION_SPEED, JSON.stringify(config.animationSpeed)],
        [THEME_STORAGE_KEYS.COLOR_SCHEME, JSON.stringify(config.colorScheme)],
        [THEME_STORAGE_KEYS.ACCESSIBILITY_SETTINGS, JSON.stringify(config.accessibilitySettings)],
        [THEME_STORAGE_KEYS.SEASONAL_ENABLED, JSON.stringify(config.seasonalEnabled)],
  ...
  const updateEffectiveTheme = useCallback(() => {
    ...
    const meshConfig = generateMeshConfig(themeConfig, currentSeason, timeOfDay);
  ...
  const applySeasonalTheme = (baseTheme, seasonalTheme) => {
    return {
      ...
      meshOverlay: getDynamicColor(seasonalTheme.primaryColor, seasonalTheme.meshOpacity),
    };
  };
  ...
  const updateMeshDensity = useCallback(async (density) => {
  ...
  const updateAnimationSpeed = useCallback(async (speed) => {
  ...
  const updateAccessibilitySettings = useCallback(async (settings) => {
    const newConfig = {
      ...themeConfig,
      accessibilitySettings: { ...themeConfig.accessibilitySettings, ...settings },
    };
    setThemeConfig(newConfig);
    await saveThemeToStorage(newConfig);
  }, [themeConfig]);
  ...
  const toggleSeasonalThemes = useCallback(async () => {
    const newConfig = { ...themeConfig, seasonalEnabled: !themeConfig.seasonalEnabled };
    setThemeConfig(newConfig);
    await saveThemeToStorage(newConfig);
  }, [themeConfig]);
  ...
  // Utility functions
  const getThemeColor = useCallback((colorKey, opacity = 1) => {
    const color = currentTheme[colorKey] || MeshColors.primaryTeal;
    return opacity === 1 ? color : getDynamicColor(color, opacity);
  }, [currentTheme]);

  const interpolateThemeColor = useCallback((color1Key, color2Key, factor) => {
  ...
  const generateUserMeshPattern = useCallback((userId) => {
    return MeshPatterns.generateUserMesh(userId, {
      ...meshPatternConfig,
      colorScheme: effectiveColorScheme,
      seasonal: themeConfig.seasonalEnabled ? currentSeason : null,
    });
  }, [meshPatternConfig, effectiveColorScheme, themeConfig.seasonalEnabled, currentSeason]);
  ...
  }, [themeConfig.animationSpeed, themeConfig.accessibilitySettings.reducedMotion]);
